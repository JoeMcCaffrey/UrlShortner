...
function shortener(alphabet, protocol, ids) {
  ...
  function getPath(url) {
    ...
    var last = url;
    var want =last.slice(last.lastIndexOf("/")+1, last.length);
    return want;
  }
  ...
  function toDigits(id) {
    ...
    do{
     ...
     numbers.push(Math.floor(remainder));
  ...
  function shorten(urll) {
    ...
    var id = nextid();
    var new_id = ""; 
    var array = toDigits(id);
  //  console.log(array);
    var url_string="";
    for(i =0; i< array.length; i++){
     new_id += array[i];
    
     url_string += alphabet[array[i]];
    }
  //  console.log(new_id);
    urlmap[new_id]= urll;     
  ...
  function expand(urls) {
    ...
    for( i = 0 ; i< path.length; i++){
      numbers_array[i] = alphabet.indexOf(path.charAt(i));
    }
    var number="";
